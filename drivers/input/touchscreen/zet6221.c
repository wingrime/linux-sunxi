...
/*for testing*/
#define CONFIG_ZET6221_TS_PEN_TIMER
...
#endif
  /* info from device*/
  u16 dev_xres; /* device max x*/
  u16 dev_yres; /* device max y*/
  int key_used;  /* is key application used */
};
...
/* zet6221 commands */
#define ZET_CMD_RESET           0xB0
#define ZET_CMD_DEEPSLEEP       0xB1
#define ZET_CMD_DEVINFO         0xB2
#define ZET_CMD_IDLE            0xB3
#define ZET_CMD_WAKEUP          0xB4
#define ZET_CMD_CHARGER_ENABLE  0xB5
#define ZET_CMD_CHARGER_DISABLE 0xB6
...
u8 x_trace_num;           /* 14     */
u8 y_trace_num;           /* 15     */
u8 dynamic_finger;        /* 16     */
u8 reserved;              /* 17     */

} zet_devinfo;
/*zet6211 ignores all stuff related i2c adresses ony byte send*/
int zet_send_cmd(u8 cmd)
{
	unsigned char write_cmd[1] = {0};
	write_cmd[0] = cmd;
	return i2c_write_interface(I2C_CTPM_ADDRESS, write_cmd, 1);
}

/*  Read and print device configuration*/
static int zet_get_devinfo()
{
  zet_devinfo inf;
  struct zet_ts_data *data = i2c_get_clientdata(this_client);
  pr_info("Device information.\n");
  /* request information */
  zet_send_cmd(ZET_CMD_DEVINFO);
  /* we wait 17-byte answer */
  zet_i2c_rxdata(&inf,17);
  pr_info("x_res=%d,y_res=%d,x_t=%d,y_t=%d,df=%d",
	  inf.x_res, inf.y_res, inf.x_trace_num,
	  inf.y_trace_num, inf.dynamic_finger);
  /* copy information to data*/
  data->dev_xres = inf.x_res;
  data->dev_yres = inf.y_res;
  data->key_used = !!(inf.dynamic_finger & 0x80);
  return 0;
}
...
static void  zet_ts_pen_irq_work(struct work_struct *work)
{
	...
	unsigned int x, y, p;
	/* we expect minimal amount of data*/
	ret = zet_i2c_rxdata(buf, 7 + data->key_used);
	if (ret < 0) {
		pr_info("%s read_data i2c_rxdata failed: %d\n", __func__, ret);
		return;
	}
	/*  check for pen release event*/
	if (!buf[1]) {
#ifndef CONFIG_ZET6221_TS_PEN_TIMER
		pr_info("pen release\n");
	...
	if (buf[0] == 0x3c ) {
	  ...
	  if ( fingers & 0x80  == 0x80) {
	    ...
	    /* check overload */
	    if (x > data->dev_xres ) x = data->dev_xres;
	    if (y > data->dev_yres ) y = data->dev_yres;
	    /* map virtual ts to screen*/
		x = (x*SCREEN_MAX_X) / data->dev_xres;
		y = (y*SCREEN_MAX_Y) / data->dev_yres;
		if (revert_y_flag)
		  y  = SCREEN_MAX_Y - y;
...
/*if device support power port feature simply poweroff ts chip */
	if (gpio_power_hdle) {
		...
		zet_send_cmd(ZET_CMD_DEEPSLEEP);
	}
}
